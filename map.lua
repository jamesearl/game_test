...
function Map:init(h, w)
	self.h = h
	self.w = w
	self.fb = love.graphics.newCanvas()
	self.camera = Camera(self)
	self.zoned = false

--	TiledMap_Load("maps/map01.tmx")
end
...
function Map:update(dt)
	...
	local s = 500*dt
    if game.keys.up then gCamY = gCamY - s end
    if game.keys.down then gCamY = gCamY + s end
    if game.keys.left then gCamX = gCamX - s end
    if game.keys.right then gCamX = gCamX + s end
